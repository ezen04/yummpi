    ...
    settlementMember: {
      ...
      member: {
        ...
        user: {
          ...
          pushSubscriptions: [] as { id: string; endpoint: string; p256dhKey: string; authKey: string }[],
        },
      },
...
function makeJob(targetUserId = 'user-1') {
  return { data: { meetingId: 'meeting-1', paymentId: 'payment-1', targetUserId } };
}

describe('processReminderJob — skip 분기', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(prisma.meeting.findUnique).mockResolvedValue({ title: '테스트 모임' } as never);
  });
  ...
  it('targetUserId 불일치이면 skip', async () => {
    vi.mocked(prisma.payment.findUnique).mockResolvedValue(makePayment() as never);
...
describe('processReminderJob — 알림 발송', () => {
  ...
  it('웹푸시 성공 시 이메일 미발송', async () => {
    ...
    payment.settlementMember.member.user.pushSubscriptions = [
      { id: 'sub-1', endpoint: 'https://push.example.com', p256dhKey: 'p256dh', authKey: 'auth' },
    ];
    vi.mocked(prisma.payment.findUnique).mockResolvedValue(payment as never);
