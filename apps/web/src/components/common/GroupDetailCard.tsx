...
export function TodoCard({ type, onAction, className }: TodoCardProps) {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onAction}
        className="h-9 rounded-[var(--radius-8)] border border-[var(--primary)] bg-transparent text-[13px] font-medium font-[var(--font-sans)] text-[var(--primary)] cursor-pointer transition-colors hover:bg-[var(--primary-tint)] active:opacity-80"
      >
        {config.action}
      </button>
    </div>
