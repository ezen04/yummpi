...
> 배치 위치: `apps/web/src/features/place/CLAUDE.md` + `components/common/CLAUDE.md` (또는 심볼릭 공유)
...
styles/tokens.css (ds/yummpi-tokens.css) , components/KakaoMapScript.tsx , hooks/useKakaoMap.ts
```
...
| 경로 | 오너 |
| --- | --- |
| `features/meeting|dashboard|auth|mypage/**`, `api/.../meetings|users|auth/**`, `prisma/schema.prisma` | ① |
| `features/vote/**`, `server.js`, `api/.../votes/**` | ③ |
...
- **전 팀이 디자인 토큰·컴포넌트를 소비**: 토큰명·컴포넌트 props 변경은 breaking change — 변경 전 전원 공지, `design.md` 동시 갱신
- 카카오 키: REST 키는 서버 전용, JS 키만 `NEXT_PUBLIC_`. 클라이언트에서 카카오 REST API 직접 호출 금지
- 후보 추가는 회원/게스트 모두 가능 — `created_by_member_id` 기준 (ERD v2.1)
...
- 모든 색·크기는 `design.md` 토큰만 사용 — HEX 하드코딩 금지
