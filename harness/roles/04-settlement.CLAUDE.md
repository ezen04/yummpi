...
> 배치 위치: `apps/web/src/features/settlement/CLAUDE.md`

## 담당

- **FE**: 영수증 업로드 · OCR 검수 · 소비 항목 선택 · 정산 결과 화면 (YSteps 4단계: 영수증→검수→선택→정산)
...
| 경로 | 오너 |
| --- | --- |
| `features/meeting|dashboard/**`, `api/.../meetings|users/**`, `prisma/schema.prisma` | ① |
| `features/place/**`, `components/common/**` | ② |
| `features/vote/**`, `server.js` | ③ |
| `features/payment/**`, `api/.../payments/**`, `packages/types/**`, `.github/**` | ⑤ |
...
- **정산은 모임당 1건** (ERD v2.1: `settlements.meeting_id` UNIQUE, receipt_id 없음). 영수증 N장은 단일 정산에 합산
