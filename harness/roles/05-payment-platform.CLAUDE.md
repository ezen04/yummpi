...
> 배치 위치: `apps/web/src/features/payment/CLAUDE.md` (+ 플랫폼 파일은 루트 레벨 오너십)

## 담당

- **FE**: 송금 화면 · 송금 현황 · 모임 종료 · PWA 셸
- **BE/플랫폼**: 송금 딥링크 · BullMQ 독촉 알림(웹푸시·메일) · **공용 Zod 계약(`packages/types`)** · CI · Playwright 하니스 · Docker/배포
...
packages/types/**  (공용 계약 오너 — 단, 변경 전 소비자 공지)
.github/** , turbo.json , docker-compose*.yml , next.config.ts(PWA) , public/manifest.json
...
| `features/vote/**`, `server.js` | ③ |
| `features/settlement/**`, `api/.../receipts|settlements/**` | ④ |

**packages/types 특칙**: 오너는 ⑤지만 스키마는 전원의 계약. 필드 추가·변경 시 (1) 소비하는 역할 파악 (2) 공지 (3) breaking change면 전원 승인 후 반영. AI는 다른 역할 폴더 작업 중 `packages/types`를 임의 수정하지 않는다 — ⑤에게 요청.
