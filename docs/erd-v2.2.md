...
| 12 | **NOTIFICATION 추가** ★ post-v2.2(#117) | F10 인앱 알림함(⑤). 적재는 알림 worker 중앙(멱등 UPSERT), 도메인 직접 INSERT 금지. `category` enum, `url`·`meeting_id`·`read_at` nullable, `dedupe_key` UNIQUE. user FK=RESTRICT(탈퇴 익명화 보존)·meeting FK=SetNull |
...
- ITEM_BASED: 일반 멤버 `settlement_members.final_amount == SUM(item_assignments.assigned_amount)`. **주최자(`role=HOST`)는 반올림 차액·미선택/OCR 총액 갭 흡수분만큼 `final_amount ≥ SUM(assigned_amount)`** (차액 = `adjustment_amount`)
- `SUM(settlement_members.final_amount) == settlements.total_amount` (전역 불변식 유지)
...
erDiagram
    ...
    MEETING ||--o{ NOTIFICATION : "about (nullable)"
    ...
    NOTIFICATION {
      uuid id PK
      uuid user_id FK
      string category "PAYMENT|SETTLEMENT|VOTE|MEETING"
      string title
      string body
      string url "nullable, 클릭 목적지"
      uuid meeting_id FK "nullable, onDelete SetNull"
      datetime read_at "nullable, null=안읽음"
      string dedupe_key UK "nullable, 적재 멱등 키"
      datetime created_at
    }
    MEETING {
      uuid id PK
...
notifications.dedupe_key                          UNIQUE (nullable, 적재 멱등)  ★ post-v2.2(#117)
notifications(user_id, created_at)                INDEX (내 알림 최신순)        ★ post-v2.2(#117)
