...
>
> **v2.2 변경 요약**: ① 카카오 로그인 NextAuth 방식으로 정정(커스텀 `login/kakao` 제거) · ② 게스트 인증 자체 토큰(b안) 확정 · ③ 예약 상태 권한 "호스트"로 통일 · ④ `users/me` 알림 설정 필드 추가 · ⑤ 모임 `votingClosesAt` 추가 · ⑥ OCR 검수 미분류 라인 노출(④ 추가 예정) · ⑦ 웹푸시 구독 API 신설(⑤ 확정)
...
- 게스트도 모임 멤버로 인증된 경우 본인 Payment에 대해 `REPORT_TRANSFER` 가능
...
- 게스트도 모임 멤버이면 회원과 동일한 Mock 송금 플로우를 사용한다.
- 호스트가 입금 확인 → `{ "action": "MARK_PAID" }`
- 호스트가 입금 확인 대기 상태로 되돌림 → `{ "action": "MARK_PENDING" }`
- 운영상 면제 → `{ "action": "MARK_EXEMPT" }`

게스트 토큰이 없거나 만료되어 모임 멤버로 해석되지 않으면 송금 화면 클라이언트는 `/meetings/:meetingId/payments/join-required` 안내 페이지로 보낸다. 이 페이지는 ⑤ 송금 화면 하위에서만 사용하며, 기존 초대 링크로 다시 입장하라는 안내만 제공한다. `meetingId`로 `inviteCode`를 역조회해 자동 redirect하지 않는다.

---

## 11-B. 웹푸시 구독 API ★ v2.2 신규 (⑤ 확정)

> F10 송금 독촉·웹푸시용. `PUSH_SUBSCRIPTION` 테이블(ERD v2.2) 기반. 회원 전용. 게스트는 MVP 독촉 알림 대상에서 제외한다.

### `POST /api/v1/push/subscriptions` — 구독 등록

요청 `{ "endpoint": string, "p256dh": string, "auth": string, "userAgent"?: string }`
- `endpoint` UNIQUE 기준 upsert (이미 있으면 p256dh/auth/userAgent 갱신 후 200, 신규면 201)
- 회원 전용. 비로그인/게스트 → 401 `UNAUTHORIZED`

### `DELETE /api/v1/push/subscriptions` — 구독 해제

요청 `{ "endpoint": string }`
- 본인 구독만 삭제 가능. 없거나 타인 구독이면 404 `PUSH_SUBSCRIPTION_NOT_FOUND`
