...
| ② 장소·지도 + 디자인시스템 | 장소 후보·지도·최적 장소 + 공통 컴포넌트·디자인 토큰 | 카카오 Local 프록시·Haversine 중간지점 |
| ③ 실시간·투표 | 투표 화면·실시간 동기화·최종 확정 | Socket.io+Redis Adapter·투표 정합성(트랜잭션·락)·장소 확정 |
...
**페어링**: 스키마(①+⑤) · Redis/Socket(③+⑤) · 카카오 키/토큰(②+⑤) · OCR/정산(④+⑤) · Socket 동시성(③+①) · BullMQ(① 만료 ↔ ⑤ 독촉).
...
- **정산 확정 후 수정**: 금액 잠금. 수정하려면 주최자가 정산을 "재오픈"해야 함.
- **영수증 다중**: 한 모임에 N장 허용(2차·추가 주문). 정산은 영수증 합산.
- **OCR 실패 fallback**: `ocrStatus: FAILED` → 수동 입력 모드 전환. fallback 없이 OCR만 의존하는 구현 금지.
- **정산 방식**: `ITEM_BASED`(소비 항목 기반) 기본 + `EQUAL`(1/N) fallback. 봉사료·세금은 기본 부담금 비율 배분, 반올림 차액은 총액 일치하도록 보정.
